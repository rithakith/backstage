...
describe('Wso2DiscoveryService', () => {
  ...
  it('should orchestrate and map settings, apis, products, mcps, and gateway discovery successfully', async () => {
    ...
    const mockServicesList = [{ id: 'service-1' }];
    ...
    const serviceEntity = { apiVersion: 'backstage.io/v1alpha1', kind: 'API', metadata: { name: 'service-1' } } as any;
    ...
    expect(mockFetchServiceList).toHaveBeenCalledWith(mockClient, logger);
    ...
    expect(logger.info).toHaveBeenCalledWith('[Wso2DiscoveryService] Discovery complete. Found 5 entities.');
