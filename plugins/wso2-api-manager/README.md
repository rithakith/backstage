...
1. `@local/backstage-plugin-wso2-api-manager` (This package)
2. `@local/backstage-plugin-wso2-api-manager-backend`
3. `@local/backstage-plugin-catalog-backend-module-wso2-apim`
4. `@local/backstage-plugin-catalog-backend-module-asgardeo` (Optional: for User/Group syncing)

### 1. Installation

```bash
# Frontend
yarn workspace app add @local/backstage-plugin-wso2-api-manager

# Backend
yarn workspace backend add @local/backstage-plugin-wso2-api-manager-backend @local/backstage-plugin-catalog-backend-module-wso2-apim
```

#### Verification
To verify the installation is successful, check your `package.json` files for the following entries:
- **Frontend**: Check `packages/app/package.json` for `@local/backstage-plugin-wso2-api-manager`.
- **Backend**: Check `packages/backend/package.json` for `@local/backstage-plugin-wso2-api-manager-backend`, etc.
