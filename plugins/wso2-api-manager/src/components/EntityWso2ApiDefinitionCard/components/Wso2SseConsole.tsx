...
export const Wso2SseConsole = ({
  ...
  const authHeaders = useMemo(() => {
    ...
    // Use ApiKey header name when API Key is enabled (as per user request)
    if (isApiKeyEnabled || apiKeyAuthPolicy || currentKey !== null) {
    ...
    return headers;
  }, [apiKeyRef, externalApiKey, apiKeyAuthPolicy, refreshToken, details, isApiKeyEnabled]);

  const getSseUrl = (path: string) => {
    if (!activeUrl) return '';
    if (path === '/*' || path === '/' || !path) {
      return activeUrl;
    }
    const formattedPath = path.startsWith('/') ? path : `/${path}`;
    return `${activeUrl.replace(/\/$/, '')}${formattedPath}`;
  };

  const getCurlCommand = (path: string) => {
    const fullUrl = getSseUrl(path);
    const headerFlags = authHeaders
      .map(hdr => `-H '${hdr.name}: ${hdr.value}'`)
      .join(' ');

    return `curl -X GET '${fullUrl}' ${headerFlags}`.trim();
  };
              ...
              {selectedUrl === 'custom' && (
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    placeholder="http://localhost:8284/events/1.0.0"
                  />
                </Grid>
              )}
            </Grid>
