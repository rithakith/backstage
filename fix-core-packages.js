...
const PACKAGES = [
  ...
  { dir: 'packages/integration', externals: ['@backstage/*', 'node-fetch', 'minimatch', 'parse-url', 'js-yaml', '@octokit/*', '@azure/*', 'agent-base', 'https-proxy-agent', 'http-proxy-agent', 'tunnel', 'cross-fetch', 'lodash', 'luxon', 'git-url-parse'] },
  { dir: 'packages/config-loader', externals: ['@backstage/*', 'js-yaml', 'json-schema', 'minimatch', 'fs-extra', 'vm2', 'typescript-json-schema', 'ajv', 'chokidar', 'minimist', 'lodash', 'json-schema-merge-allof', 'json-schema-traverse'] },
...
(async () => {
  ...
  console.log('\nDone! Now run:');
  console.log('  yarn workspace @local/backstage-plugin-wso2-api-manager build');
})();
